...
# 保存图片
pdf(
  "/root/wangje/Project/刘老师/NK_T/Data/CCA新的分群/new_CD4/cd4_T细胞celltype堆叠柱形图.pdf",
   height = 8,width = 15,family = 'ArialMT')
p3
dev.off()
```
...
# CD4 T cells
cd8_df <- scRNA@meta.data %>% dplyr::filter(celltype2 %in% cell1s) %>% select(celltype2, Treat_assess, cloneType)
...
p3 <- ggplot() +
  ...
  geom_vline(xintercept = 3.84, lty="dashed", color = "grey50", linewidth = 0.8)
```
[![piQpc5T.png](https://z1.ax1x.com/2023/11/04/piQpc5T.png)](https://imgse.com/i/piQpc5T)
